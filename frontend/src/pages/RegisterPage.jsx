...
const RegisterPage = () => {
  ...
  const inputClass =
    'w-full px-3.5 py-2.5 text-sm text-white bg-white/[0.04] border border-border-subtle rounded-lg outline-none focus:border-accent/50 focus:bg-white/[0.06] focus:shadow-[0_0_0_3px_rgba(139,92,246,0.08)] transition-all placeholder:text-text-muted';
  ...
  return (
    <div className="flex items-center justify-center min-h-screen p-8 relative overflow-hidden">
      ...
      {/* Card */}
      <div className="relative z-10 w-full max-w-[420px] p-10 glass-strong rounded-3xl shadow-2xl shadow-accent/5 animate-fade-up" id="register-card">
        <div className="text-center mb-8">
          <span className="inline-block text-4xl text-accent mb-3 animate-pulse-glow">◈</span>
          <h1 className="text-2xl font-bold mb-1">Create account</h1>
          <p className="text-sm text-text-dim">Start your focused study journey</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4" id="register-form">
          <div className="flex flex-col gap-1">
            <label htmlFor="register-name" className="text-xs font-medium text-text-dim">Name</label>
            <input type="text" id="register-name" className={inputClass} placeholder="Your name (3-10 chars)"
              value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} required />
          </div>
          <div className="flex flex-col gap-1">
            <label htmlFor="register-username" className="text-xs font-medium text-text-dim">Username</label>
            <input type="text" id="register-username" className={inputClass} placeholder="Choose a username (4+ chars)"
              value={form.username} onChange={(e) => setForm({ ...form, username: e.target.value })} required />
          </div>
          <div className="flex flex-col gap-1">
            <label htmlFor="register-email" className="text-xs font-medium text-text-dim">Email</label>
            <input type="email" id="register-email" className={inputClass} placeholder="you@example.com"
              value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} required />
          </div>
          <div className="flex flex-col gap-1">
            <label htmlFor="register-password" className="text-xs font-medium text-text-dim">Password</label>
        ...
        <p className="text-center mt-6 text-sm text-text-dim">
          ...
          <Link to="/login" className="text-accent font-medium hover:text-accent-light transition-colors">Sign in</Link>
        </p>
      </div>
    </div>
  );
